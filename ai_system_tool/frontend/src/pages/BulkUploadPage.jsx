  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
          isDragActive
            ...
            : 'border-[#cbd5e1] bg-[#ffffff] hover:border-[#6366f1]/50'
        }`}
      >
        <input {...getInputProps()} />
        <div className="flex flex-col items-center gap-3">
          <div className={`p-3 rounded-full ${isDragActive ? 'bg-[#6366f1]/20' : 'bg-[#f1f5f9]'}`}>
          ...
          {isDragActive ? (
            ...
            <>
              <p className="text-[#334155] font-medium">Drag & drop multiple files here</p>
      ...
      {files.length > 0 && !results && (
        <div className="bg-[#ffffff] border border-[#f1f5f9] rounded-xl p-4 space-y-2">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-[#0f172a]">{files.length} file(s) selected</span>
            <button
              onClick={clearAll}
          ...
          {files.map((file, idx) => (
            <div key={idx} className="flex items-center justify-between p-2 bg-[#f1f5f9] rounded-lg">
              <div className="flex items-center gap-3">
                <div className="text-[#6366f1]">{getFileIcon(file)}</div>
                <div>
                  <p className="text-[#0f172a] text-sm truncate max-w-[300px]">{file.name}</p>
      ...
      {uploading && (
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-[#475569]">Uploading {files.length} files...</span>
      ...
      {results && (
        <div className="bg-[#ffffff] border border-[#f1f5f9] rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-[#0f172a]">Upload Results</h3>
          ...
          {results.results?.map((r, idx) => (
            <div
              ...
              className={`flex items-center justify-between p-2 rounded-lg ${
                r.status === 'queued' ? 'bg-[#f1f5f9]' : 'bg-[#ef4444]/5'
              }`}
            >
              <div className="flex items-center gap-3">
                {r.status === 'queued' ? (
                ...
                <span className="text-[#0f172a] text-sm">{r.filename}</span>
              </div>
          ...
          <button
            onClick={clearAll}
            className="w-full py-2 rounded-lg text-sm text-[#6366f1] hover:text-[#818cf8] border border-[#f1f5f9] hover:border-[#6366f1]/30 transition-all"
          >
            Upload More
          </button>
        </div>
